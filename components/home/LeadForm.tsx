...
const STORAGE_KEY = 'lead_orcamento';
...
export const LeadForm: React.FC = () => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    window.location.href = buildUrl(ROUTES.chat, { intent: 'orcamento' });
  };
    ...
    <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-3xl p-6 md:p-10 shadow-elevation-1 space-y-6">
      <div className="grid md:grid-cols-2 gap-5">
        <div className="space-y-2">
          <label className="text-xs font-display font-bold uppercase tracking-[0.2em] text-slate-500">Nome do comprador</label>
          <input
            value={form.name}
        ...
        <div className="space-y-2">
          <label className="text-xs font-display font-bold uppercase tracking-[0.2em] text-slate-500">Empresa (Razao Social)</label>
          <input
            value={form.company}
        ...
        <div className="space-y-2">
          <label className="text-xs font-display font-bold uppercase tracking-[0.2em] text-slate-500">CNPJ (opcional)</label>
          <input
            value={form.cnpj}
        ...
        <div className="space-y-2">
          <label className="text-xs font-display font-bold uppercase tracking-[0.2em] text-slate-500">Segmento</label>
          <select
            value={form.segment}
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-xs font-display font-bold uppercase tracking-[0.2em] text-slate-500">WhatsApp</label>
          <input
            value={form.whatsapp}
        ...
        <div className="space-y-2 md:col-span-2">
          <label className="text-xs font-display font-bold uppercase tracking-[0.2em] text-slate-500">Lista de itens de interesse</label>
          <textarea
            value={form.items}
