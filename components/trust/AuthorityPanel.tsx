...
export const AuthorityPanel: React.FC<AuthorityPanelProps> = ({
  ...
  return (
    <div className={cn('bg-bg-surface border border-border-subtle rounded-2xl p-8 md:p-10 shadow-elevation-1', className)}>
      ...
      <h3 className="text-2xl md:text-3xl font-display font-semibold text-text-primary mt-3">{title}</h3>
      <p className="text-text-body text-sm md:text-base leading-relaxed mt-3">{description}</p>

      <div className="grid md:grid-cols-3 gap-6 mt-8">
        <div className="space-y-3">
          <p className="text-[11px] font-display font-semibold uppercase tracking-[0.18em] text-text-muted">{leadLabel}</p>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-display font-semibold text-text-primary">{AUTHORITY_INFO.leadName}</p>
              <p className="text-xs text-text-muted">{AUTHORITY_INFO.leadRole}</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {AUTHORITY_INFO.credentials.map((item) => (
              <span key={item} className="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-bg-surfaceMuted text-text-body">
                {item}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-3">
          <p className="text-[11px] font-display font-semibold uppercase tracking-[0.18em] text-text-muted">{sourcesLabel}</p>
          <div className="flex flex-wrap gap-2">
