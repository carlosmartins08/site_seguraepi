...
export const PartnerShowcase: React.FC<PartnerShowcaseProps> = ({
  ...
  return (
    <Section id={id} variant="offwhite" className="py-20 md:py-28">
      <Container>
        <div className="bg-white rounded-4xl border border-white/40 shadow-segura-soft px-6 py-10 md:px-10 md:py-14">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 md:gap-8">
            <div className="space-y-3 max-w-3xl">
              <p className="text-segura-primary font-display font-bold uppercase tracking-[0.3em] text-[10px]">
                Curadoria Técnica
              </p>
              <h3 className="text-3xl md:text-4xl font-display font-black text-segura-dark uppercase tracking-tight">
                {title}
              </h3>
              <p className="text-slate-600 text-lg leading-relaxed">{subtitle}</p>
            </div>
            <div className="flex items-center gap-3 bg-slate-900 text-white px-4 py-3 rounded-2xl shadow-segura-soft">
              <span className="w-10 h-10 rounded-xl bg-segura-primary/20 text-segura-primary flex items-center justify-center">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {partners.map((partner) => (
                <a
                  ...
                  className={cn(
                    'group relative overflow-hidden rounded-3xl border border-slate-100 bg-slate-900 text-white p-5 flex flex-col gap-3 shadow-segura-soft transition-all duration-300 min-h-[110px]',
                    'hover:-translate-y-1 hover:shadow-segura-glow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-segura-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white'
                  )}
                ...
                <span className="text-sm font-display font-bold uppercase tracking-[0.2em] text-slate-200 group-hover:text-white transition">
                  ...
                  {partner.caValidUntil && (
                    <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.2em] text-slate-300">
                      <SeguraIcon name="clock" className="w-4 h-4 text-segura-primary" />
                      CA válido até {partner.caValidUntil}
                    </div>
                  )}
                  <div className="absolute inset-0 pointer-events-none bg-gradient-to-br from-segura-primary/0 via-white/5 to-segura-primary/0 opacity-0 group-hover:opacity-100 transition duration-500" />
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <a
              href={ctaHref}
              className="inline-flex items-center gap-2 text-sm font-display font-bold uppercase tracking-[0.2em] text-segura-dark hover:text-segura-primary transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-segura-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
            >
              {ctaLabel}
            </a>
          </div>
